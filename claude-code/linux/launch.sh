...
usage() {
  cat <<'EOF'
Claude Code 安全起動 (Linux)

使い方:
  ./claude-code/linux/launch.sh [オプション]
...
if [[ "$ALLOW_DANGEROUS" -eq 1 ]]; then
  log_warn "全権限オプション (--dangerously-skip-permissions) を有効化します。利用者はリスクを理解している必要があります。"
fi

# 6. 承認と実行
declare -a CMD=(claude)
if [[ "$ALLOW_DANGEROUS" -eq 1 ]]; then
...
if [[ "$YES" -ne 1 ]]; then
  log_info "Claude Code を起動しますか? (yes/no)"
  read -r answer
  case "$answer" in
    y|Y|yes|YES) ;;
