...
.DESCRIPTION
起動前検査 (Git 状態・指示ファイル・プロンプト変数) を実施してから Claude Code を起動します。
既定では起動前に確認を求めます。
...
.EXAMPLE
./claude-code/windows/Start-ClaudeCode.ps1 -ProjectDirectory C:\work\sample -WhatIf
#>
[CmdletBinding()]
param(
...
if ($WhatIf) {
    Write-Host "[INFO] 実行予定コマンド: claude $($argsList -join ' ')"
    exit 0
}
...
if (-not $Yes) {
    $answer = Read-Host 'Claude Code を起動しますか? (yes/no)'
...
Write-Host "[INFO] Claude Code を起動します: $ProjectDirectory"
