...
| 4 | 障害通知は Webhook 設定時のみ動作 | 中→**解消** | Bats + **実地（systemd OnFailure → Webhook）確認済み（2026-08-12）** | — |
| 5 | Windows のジャンクション対策は CI（Windows 実機）で検証する必要あり | 中 | 文字列ベース検証 + 実装 | PR マージ後の CI で確認 |
| 6 | 実 CLI での AI 実行（トークン消費を伴う本番セッション）E2E | 中→**解消** | 検証環境で Claude Code の実セッション成功（output.txt 作成・exit 0） | — |
